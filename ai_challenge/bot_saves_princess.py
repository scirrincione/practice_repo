...
def displayPathtoPrincess(n,grid):
    ...
    while index_m != index_p:
        ...
        if distance_calc((index_m[1]-1, index_m[0]), index_p) <= lowest:
            index_m = (index_m[1]-1, index_m[0])
            moves += "LEFT\n"
            curr_distance = distance_calc(index_m, index_p)
            lowest = curr_distance
        if distance_calc((index_m[1]+1, index_m[0]), index_p) <= lowest:
            ...
            curr_distance = distance_calc(index_m, index_p)
            lowest = curr_distance
        if distance_calc((index_m[1], index_m[0]-1), index_p) <= lowest:
            index_m = (index_m[1], index_m[0]-1)
            moves += "UP\n"
            curr_distance = distance_calc(index_m, index_p)
            lowest = curr_distance
        if distance_calc((index_m[1], index_m[0]+1), index_p) <= lowest:
